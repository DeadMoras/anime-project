    ...
    public function getAuth(array $data)
    {
        ...
        if ( ! $user) {
            return response()->json(
                $error->changeErrorTrue()
                    ->addObject('error_code', 401)
                    ->addObject('error_data', 'Проверьте правильность данных'), 401);
        }
        ...
        if ($user->confirmed == 0) {
            return response()->json(
                $error->changeErrorTrue()
                    ->addObject('error_code', 401)
                    ->addObject('error_data', 'Вы не активировали аккаунт'), 401);
        }
        ...
        if (9 < strlen($user->remember_token)) {
            return response()->json(
                $error->changeErrorTrue()
                    ...
                    ->addObject('error_data', 'Вы уже авторизованы'), 406);
        }
        ...
        if (password_verify($values[1], $user->password)) {
            ...
            return response()->json(
                $error->changeErrorTrue()
                    ->addObject('error_code', 401)
                    ->addObject('error_data', 'Проверьте правильность пароля'), 401);
        }
    }
}